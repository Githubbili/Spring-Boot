...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  ,tables = {
    ...
    ,cache: {} //数据缓存
    ,index: layui.tables ? (layui.tables.index + 10000) : 0
  ...
  //字符常量
  ,MOD_NAME = 'tables', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
  ...
  ,Class = function(options){
    var that = this;
    that.index = ++tables.index;
    that.config = $.extend({}, that.config, tables.config, options);
    that.render();
  };
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        if(!sort){
          item1[tables.config.indexName] = i1;
        }
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
              ...
              if(item3.type === 'checkbox'){
                return '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" '+ function(){
                  var checkName = tables.config.checkName;
                  //如果是全选
                  if(item3[checkName]){
    ...
    tables.cache[that.key] = data; //记录数据
  ...
  Class.prototype.sort = function(th, type, pull, formEvent){
    ...
    ,data = tables.cache[that.key], thisData;
  ...
  Class.prototype.setCheckData = function(index, checked){
    var that = this
    ,options = that.config
    ,thisData = tables.cache[that.key];
  ...
  Class.prototype.syncCheckAll = function(){
    ...
    if(tables.checkStatus(that.key).isAll){
  ...
  Class.prototype.events = function(){
    ...
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){
      ...
      layui.event.call(this, MOD_NAME, 'checkbox('+ filter +')', {
        checked: checked
        ,data: tables.cache[that.key] ? (tables.cache[that.key][index] || {}) : {}
        ,type: isAll ? 'all' : 'one'
      });
    });
    ...
    that.layBody.on('change', '.'+ELEM_EDIT, function(){
      ...
      ,data = tables.cache[that.key][index];
    ...
    that.layBody.on('click', '*[lay-event]', function(){
      ...
      layui.event.call(this, MOD_NAME, 'tool('+ filter +')', {
        data: tables.clearCacheKey(data)
        ,event: othis.attr('lay-event')
        ,tr: tr
        ,del: function(){
          tables.cache[that.key][index] = [];
          tr.remove();
          that.scrollPatch();
        }
  ...
  //初始化
  tables.init = function(filter, settings){
    settings = settings || {};
    var that = this
    ...
    elemTable.each(function(){
      ...
      }, tables.config, settings, tableData);
  ...
  //表格选中状态
  tables.checkStatus = function(id){
    var nums = 0
    ,invalidNum = 0
    ,arr = []
    ,data = tables.cache[id] || [];
    //计算全选个数
    layui.each(data, function(i, item){
      ...
      if(item[tables.config.checkName]){
        nums++;
        arr.push(tables.clearCacheKey(item));
      }
    });
    return {
      data: arr //选中的数据
  ...
  //表格重载
  thisTable.config = {};
  tables.reload = function(id, options){
    ...
    return tables.render($.extend(true, {}, config, options));
  };
 
  //核心入口
  tables.render = function(options){
    var inst = new Class(options);
    return thisTable.call(inst);
  };
  
  //清除临时Key
  tables.clearCacheKey = function(data){
    data = $.extend({}, data);
    delete data[tables.config.checkName];
    delete data[tables.config.indexName];
    return data;
  };
